...
def rfunc(triple_list, ent_num, rel_num):
    ...
    r_mat_ind = list()
    r_mat_val = list()
    head_r = np.zeros((ent_num, rel_num)) # hr
    tail_r = np.zeros((ent_num, rel_num)) # tr
    ...
    # r_mat: each pair of e is connected by which relation (triplet info)
    return head, tail, head_r, tail_r, r_mat


def get_mat(triple_list, ent_num):  # pos: triplet without relation, with self connection. degree: degree of each e.
    degree = [1] * ent_num
    pos = dict()
    for triple in triple_list:
        ...
        if triple[0] == triple[2]:
            continue
        if (triple[0], triple[2]) not in pos:  # with inverse relation
...
def get_sparse_tensor(triple_list, ent_num):  # M_arr: adj with 0 and 1. pos: sparse adj, weight is normalized
...
class Layer:
    ...
    def add_self_att_layer(self, inlayer, adj):
        in_fts = tf.layers.conv1d(tf.expand_dims(
            inlayer, 0), self.dim, 1, use_bias=False)  # c
        f_1 = tf.reshape(tf.layers.conv1d(in_fts, 1, 1), (-1, 1))
        f_2 = tf.reshape(tf.layers.conv1d(in_fts, 1, 1), (-1, 1))
        logits = f_1 + tf.transpose(f_2)
        adj_tensor = tf.constant(adj, dtype=tf.float32)  # w_ij
        logits = tf.multiply(adj_tensor, logits)  # w^r_{ij}a^r[c_i||c_j]
        bias_mat = -1e9 * (1.0 - (adj > 0))
        coefs = tf.nn.softmax(tf.nn.leaky_relu(logits) + bias_mat)  # alpha_ij
    ...
    def compute_r(self, inlayer):
        ...
        L = tf.matmul(head_l, inlayer) / \
            tf.expand_dims(tf.reduce_sum(head_l, axis=-1), -1) # ave head
        R = tf.matmul(tail_l, inlayer) / \
            tf.expand_dims(tf.reduce_sum(tail_l, axis=-1), -1) # ave tail
        r_embeddings = tf.concat([L, R], axis=-1)
        return r_embeddings

    def get_dual_input(self, inlayer):
        dual_X = self.compute_r(inlayer) # relation emb
        count_r = len(self.head)
        dual_A = np.zeros((count_r, count_r)) # adj of dual graph
        for i in range(count_r):
            for j in range(count_r):
                a_h = len(self.head[i] & self.head[j]) / len(self.head[i] | self.head[j]) #H(r_i, r_j)
                a_t = len(self.tail[i] & self.tail[j]) / len(self.tail[i] | self.tail[j]) #T(r_i, r_j)
                dual_A[i][j] = a_h + a_t  # w_ij
    ...
    def build(self):
        ...
        primal_X_0 = self.get_pretrained_input(self.pretrianed_embedding)  # e initial with name
        self.M = get_sparse_tensor(self.triple_list, self.ent_num)
        # M_arr: adj with 0 and 1. M: sparse adj, weight is normalized
        self.head, self.tail, self.head_r, self.tail_r, self.r_mat = rfunc(self.triple_list, self.ent_num, self.rel_num)
        # head: head of each r; tail: tail of each r; # r_mat: each pair of e connected by which relation (triplet info)
        dual_X_1, dual_A_1 = self.get_dual_input(primal_X_0)  # X: relation emb. A: w_ij of r
        ...
        output_layer = self.highway(gcn_layer_1, gcn_layer_2)  # e_num*e_dim
...
class RDGCN(BasicModel):
    ...
    def init(self):
        ...
        import joblib
        save_suffix = self.args.training_data.split(sep='/')[1]
        if os.path.exists(save_suffix + '_local_name_vectors'):
            self.local_name_vectors = joblib.load(save_suffix + '_local_name_vectors')
        else:
            _, _, self.local_name_vectors = self._get_desc_input()
            joblib.dump(self.local_name_vectors, save_suffix + '_local_name_vectors')
    ...
    def eval_ref_sim_mat(self, csls=0):
        refs1_embeddings = tf.nn.embedding_lookup(self.output, self.ref_ent1)
        refs2_embeddings = tf.nn.embedding_lookup(self.output, self.ref_ent2)
    ...
    def launch_rdgcn_training_k_repo(self, iter, iter_nums, neg_left, neg2_right):
        for i in range(1, iter_nums + 1):
            ...
            _, batch_loss = self.session.run([self.optimizer, self.loss], feed_dict=self.feeddict)
            print('epoch {}, avg. relation triple loss: {:.4f}, cost time: {:.4f}s'.format(epoch, batch_loss,
                                                                                           time.time() - start))
    ...
    def trainning(self):
        ...
        # i think this is an error, neg sampling should be inside the loop, because self.output is updated each loop
        for i in range(1, self.args.max_epoch + 1):
            ...
            if i >= self.args.start_valid and i % self.args.eval_freq == 0:
                flag = self.valid(self.args.stop_metric)[0]  # flag: valid hit@1 result
                self.flag1, self.flag2, self.early_stop = early_stop(self.flag1, self.flag2, flag)  # if valid result decrease, should early stop.
                if self.early_stop or i == self.args.max_epoch:
                    break

    def test(self, save=True):
        embedding = self.session.run(self.output)

...
def greedy_alignment(embed1, embed2, top_k, nums_threads, metric, normalize, csls_k, accurate):
    ...
    # num = len(embed1)
    # batch_size = 1000
    # batch_num = int(np.ceil(len(embed1) / batch_size))
    # hits = [0] * len(top_k)
    # mr, mrr = 0, 0
    # alignment_rest = set()
    # for i in range(batch_num):
    #     data_idxes = list(range(batch_size * i, min(batch_size * (i + 1), len(embed1))))
    #     start_idx = i * batch_size
    #     data_idxes_arr = np.array(data_idxes)
    #     sim_mat = sim(embed1[batch_size * i:min(batch_size * (i + 1), len(embed1))], embed2, metric=metric,
    #                   normalize=normalize, csls_k=csls_k)
    #     sub_num = sim_mat.shape[0]
    #     rests = list()
    #     if nums_threads > 1:
    #         search_tasks = task_divide(np.array(range(sub_num)), nums_threads)
    #         pool = multiprocessing.Pool(processes=len(search_tasks))
    #         for task in search_tasks:
    #             mat = sim_mat[task, :]
    #             idxes = data_idxes_arr[task]
    #             rests.append(pool.apply_async(calculate_rank, (idxes, mat, top_k, accurate, num)))
    #         pool.close()
    #         pool.join()
    #         for rest in rests:
    #             sub_mr, sub_mrr, sub_hits, sub_hits1_rest = rest.get()
    ...
    #             hits += np.array(sub_hits)
    #             # alignment_rest |= sub_hits1_rest
    #             sub_hits1_rest_revise = []
    #
    #             for p in sub_hits1_rest:
    #                 sub_hits1_rest_revise.append((start_idx + p[0], p[1]))
    #             alignment_rest |= set(sub_hits1_rest_revise)
    #     else:
    #         sub_mr, sub_mrr, sub_hits, sub_hits1_rest = calculate_rank(data_idxes, sim_mat, top_k, accurate, num)
    #         mr += sub_mr
    #         mrr += sub_mrr
    #         hits += np.array(sub_hits)
    #         # alignment_rest |= sub_hits1_rest
